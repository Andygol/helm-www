...
Helm включає наступні функції дати, які можна використовувати в шаблонах:
...
(mustToDate)](#todate-musttodate), та [unixEpoch](#unixepoch).
...
Перетворює тривалість на дні як число з рухомою комою.
...
Перетворює тривалість на години як число з рухомою комою.

**Еквівалент у Go:** `time.Duration.Hours()`
...
Перетворює тривалість на мікросекунди як ціле число.

**Еквівалент у Go:** `time.Duration.Microseconds()`
...
Перетворює тривалість на мілісекунди як ціле число.

**Еквівалент у Go:** `time.Duration.Milliseconds()`
...
Перетворює тривалість на хвилини як число з рухомою комою.

**Еквівалент у Go:** `time.Duration.Minutes()`
...
Перетворює тривалість на наносекунди як ціле число.

**Еквівалент у Go:** `time.Duration.Nanoseconds()`
...
Округлює тривалість до найближчого кратного іншій тривалості. Перший аргумент — тривалість, яку потрібно округлити, а другий — кратність.

**Еквівалент у Go:** `time.Duration.Round()`
...
Перетворює тривалість на секунди як число з рухомою комою.

**Еквівалент у Go:** `time.Duration.Seconds()`
...
Обрізає тривалість у бік нуля до кратного іншій тривалості. Перший аргумент — тривалість, яку потрібно обрізати, а другий — кратність.

**Еквівалент у Go:** `time.Duration.Truncate()`
...
Перетворює тривалість на тижні як число з рухомою комою.
...
`mustToDuration` перетворює значення, схоже на тривалість, на `time.Duration` і спричиняє помилку шаблону в разі помилки.

**Еквівалент у Go:** `time.ParseDuration`, конструювання `time.Duration`

Підтримувані вхідні дані включають рядки тривалості Go (наприклад, `"2h30m"`), числові рядки, які трактуються як секунди (наприклад, `"1.5"`), та числові значення, які трактуються як секунди.
