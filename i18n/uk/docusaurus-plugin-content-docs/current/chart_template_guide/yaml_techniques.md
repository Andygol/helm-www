...
Коли ви вставляєте значення чартів у маніфести, також прочитайте [Запобігання YAML-інʼєкціям під час вставлення значень Helm](#prevent-yaml-injection-when-inserting-helm-values), щоб рядки, надані користувачем, не могли змінити структуру документа.
...
### Керування пробілами в багаторядкових рядках {#controlling-spaces-in-multi-line-strings}
...
:::info
Щоб ділитися фрагментами коду між чартами, використовуйте [бібліотечні чарти](/topics/library_charts.md) замість якорів YAML. Бібліотечні чарти призначені для повторного використання та не схильні до описаної вище проблеми round-trip.
:::

## Запобігання YAML-інʼєкціям під час вставлення значень Helm {#prevent-yaml-injection-when-inserting-helm-values}

Значення чартів часто надходять із ненадійних джерел або від кількох орендарів (multi-tenant). Якщо ви вставляєте значення в шаблон за допомогою сирого `{{ .Values.foo }}`, значення, що містить символи нового рядка, індикатори мап `:` або маркери списків, може змінити структуру відрендереного YAML. Це називається _YAML-інʼєкцією_. YAML-інʼєкція може не лише зламати встановлення, але й додати зайві ключі в маніфест Kubernetes.

### Найкращі практики {#best-practices}

Щоб уникнути YAML-інʼєкції під час вставлення значень Helm, віддавайте перевагу помічникам, які кодують або структурують дані за вас, як показано в наступній таблиці:

| Мета | Рекомендовано | Уникайте |
| ---- | ------ | ----- |
| Взяти рядковий скаляр у лапки | `{{ .Values.name \| quote }}` | `{{ .Values.name }}` у полі без лапок |
| Вставити рядок як один YAML-скаляр | `quote`, або заголовок блокового скаляра (`\|` / `>`) разом із `{{- .Values.config \| nindent N }}`, де `N` більший за відступ батьківського елемента (зазвичай батьківський + 2) | Голий `{{ .Values.config }}` або `nindent` без заголовка блоку — новий рядок у значенні може почати новий ключ |
| Вбудувати мапи / списки (колекції) | `{{ toYaml .Values.extraEnv \| nindent N }}`, де `N` більший за відступ батьківського елемента (зазвичай батьківський + 2), тож `toYaml` генерує колекцію, а `nindent` лише зсуває її на місце | `{{ .Values.extraEnv \| nindent N }}` без `toYaml`, `N`, що не більший за відступ батьківського елемента, або цикли `key: {{ . }}`, написані вручну |

Нижче описано загальні найкращі практики запобігання YAML-інʼєкції під час вставлення значень:

- Ставтеся до кожного виразу `{{ ... }}`, який рендериться всередині YAML, як до ненадійного вводу.
- Використовуйте `quote`, `toYaml` та `nindent`/`indent` разом, щоб зберегти структуру YAML. Уникайте ручного створення YAML за допомогою конкатенації рядків.
- Запускайте `helm template` та перевірку схеми в CI, щоб некоректні значення були відхилені ще до того, як потраплять у кластер.

### Приклади {#examples}

#### Безпечний вкладений обʼєкт (перетворення мапи на YAML, потім відступ) {#safe-nested-object-map-to-yaml-then-indent}
...
data:
  # values.config є мапою; toYaml генерує вкладений YAML, nindent лише відступає його
  config.yaml: |
    {{- toYaml .Values.config | nindent 4 }}
```

Рядок під блоковим скаляром — та сама ідея: залиште `{{-`, щоб не отримати зайвий порожній рядок, і оберіть `N` більший за відступ батьківського елемента:
...
#### Небезпечні патерни {#unsafe-patterns}

```yaml
# ПОГАНО: значення "x\n  evil: true" стає додатковим ключем
data:
  app.conf: {{ .Values.appConf }}

# ПОГАНО: nindent без заголовка блочного скаляра лише додає пробіли
data:
  app.conf: {{ .Values.appConf | nindent 4 }}
```

Для отримання додаткової інформації дивіться [Функції шаблонів та конвеєри](functions_and_pipelines.mdx), [Відступи та шаблони](#indenting-and-templates) на цій сторінці та [Список функцій шаблонів](function_list.mdx) (`toYaml`, `quote`, `nindent`).
