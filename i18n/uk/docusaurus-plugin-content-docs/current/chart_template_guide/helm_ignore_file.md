...
Якщо цей файл існує, Helm ігнорує кожен шлях, що відповідає шаблону, під час завантаження чарту. Це стосується як `helm package`, так і команд, які рендерять або встановлюють чарт безпосередньо з теки, зокрема `helm install`, `helm upgrade` та `helm template`.

Використовуйте `.helmignore`, щоб не включати непотрібні або конфіденційні файли до запакованого чарту (а також під час локальних запусків встановлення чи оновлення з теки чарту).
