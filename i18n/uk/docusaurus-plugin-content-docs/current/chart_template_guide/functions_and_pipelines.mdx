...
Конвеєри також корисні під час роботи зі списками. Наприклад, функція `join` може обʼєднати список рядків в один рядок із роздільником:
...
Наведений вище приклад створює такий результат:
