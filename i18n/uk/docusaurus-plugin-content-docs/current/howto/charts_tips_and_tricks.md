...
## Створення відтворюваних архівів чартів {#build-reproducible-chart-archives}

Типово, файли всередині упакованого архіву чарту (`.tgz`) зберігають час модифікації вихідних файлів на диску. У результаті повторне пакування того самого чарту створює архіви, які відрізняються від однієї збірки до іншої. Щоб зробити архіви чартів відтворюваними, Helm враховує змінну середовища `SOURCE_DATE_EPOCH`, визначену проєктом [Reproducible Builds](https://reproducible-builds.org/docs/source-date-epoch/). Коли ви її встановлюєте, Helm позначає кожен файл в архіві часом модифікації, який ви надали, замість часу збірки.

Встановіть `SOURCE_DATE_EPOCH` як мітку часу Unix, кількість цілих секунд з 1970-01-01 за UTC, і запустіть `helm package`:

```console
$ SOURCE_DATE_EPOCH=1609459200 helm package ./mychart
```

Helm позначає кожен файл в отриманому архіві часом 2021-01-01T00:00:00Z, тому повторне пакування того самого вихідного коду створює однакові мітки часу. Щоб відстежувати історію вашого вихідного коду, отримайте мітку часу з вашого останнього коміту:

```console
$ SOURCE_DATE_EPOCH=$(git log -1 --pretty=%ct) helm package ./mychart
```

Майте на увазі наступне, коли використовуєте `SOURCE_DATE_EPOCH`:

- Значення має бути невідʼємним цілим числом. Helm інтерпретує його як цілі секунди за UTC і не підтримує точність менше секунди. Нечислове або відʼємне значення призводить до помилки команди `invalid SOURCE_DATE_EPOCH`.
- Коли змінна не встановлена або порожня, Helm зберігає стандартну поведінку, де записи архіву відображають фактичний час модифікації файлів.
- Коли ви пакуєте чарт, що містить файл `Chart.lock`, Helm також позначає мітку часу `generated:` цього файлу значенням `SOURCE_DATE_EPOCH`, нормалізованим до UTC і цілих секунд. У результаті чарти, які декларують залежності, створюють байт-ідентичні архіви на різних збірках і машинах, коли ви використовуєте те саме значення `SOURCE_DATE_EPOCH`, оскільки збігається вміст `Chart.lock`, а не лише час модифікації файлів.
- `helm install`, `helm upgrade`, `helm dependency build` та `helm dependency update` також враховують `SOURCE_DATE_EPOCH`, але лише коли вони повторно архівують залежність з локального репозиторію `file://`. Helm зберігає віддалено завантажені залежності так, як їх було отримано.
