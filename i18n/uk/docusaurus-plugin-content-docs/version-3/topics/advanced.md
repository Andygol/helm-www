...
## Дозволити Helm володіти наявними ресурсами{#allow-helm-to-take-ownership-of-existing-resources}

За звичай Helm відмовляється встановлювати або оновлювати чарт, якщо ресурс, який не належить до релізу, вже існує у кластері.
У такому випадку Helm виведе наступну помилку: "resource already exists and is not owned by this release".

Helm відстежує права власності на ресурси у кластері за допомогою анотацій, керованих Helm, та повʼязаних міток, таких як `app.kubernetes.io/managed-by`. Цей процес валідації права власності захищає вас від випадкового перезапису обʼєктів, створених іншим інструментом або релізом.

Використовуйте `--take-ownership` у `helm install` або `helm upgrade`, коли ви дійсно хочете, щоб Helm прийняв ці наявні обʼєкти у реліз:
...
Коли прапорець встановлено, Helm ігнорує перевірку власності ресурсів і приймає наявні обʼєкти у реліз. Подальші оновлення та видалення потім вважають ці обʼєкти частиною релізу.

### Коли це використовується {#when-to-use-it}

Типові випадки використання прапорця `--take-ownership`:

- Перенесення вручну застосованого маніфеста (або іншого інструмента розгортання) до чарту Helm без видалення живих ресурсів спочатку
- Відновлення після видалення анотацій або втрати історії релізу, так що Helm більше не розпізнає свої обʼєкти
- Узгодження чарту з кластерними обʼєктами, які вже відповідають бажаним назвам/видам

### Обмеження та найкращі практики {#limitations-and-best-practices}

- Отримання прав власності не призводить до видалення зайвих полів, яких ніколи не було в чарті. Тристороннє стратегічне об’єднання (або застосування на стороні сервера, якщо ви ним користуєтеся) як і раніше визначає, як поєднуються значення в режимі реального часу та значення з чарту.
- Щоб уникнути отримання права власності на обʼєкти, якими Helm не повинен управляти, обережно ставтеся до назв і перегляньте результати тестового запуску спочатку (`--dry-run`).
- `helm template` також приймає `--take-ownership` для паритету прапорців; він не взаємодіє з кластером.
